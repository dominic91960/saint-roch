...
const MainCategories = () => {
  return (
    <section className="container mx-auto px-[15px] text-[14px] sm:text-[17px] md:text-[20px] lg:text-[22px] xl:text-[24px] 2xl:text-[27px]">
      {categories.map(({ name, slogan, boilerImage, desc, boilerNames }) => (
        <React.Fragment key={name}>
          {/* Category name */}
          <h1 className="title-text-stroke text-[48px] capitalize leading-[0.9em] sm:text-[76px] md:text-[104px] lg:text-[132px] xl:text-[160px] 2xl:text-[196px]">
            {name.split(" ")[0]} <br />
            {name.split(" ")[1]}
          </h1>
          ...
          {/* Grid */}
          <div className="mb-[3em] mt-[4.5em] grid justify-items-center gap-[1.3em] lg:grid-cols-2 lg:items-center lg:gap-0">
            {/* Image area */}
            ...
            <div>
              ...
              {/* Types */}
              <ul className="list-inside ps-[4ch] font-medium *:flex *:items-center *:gap-[0.3em]">
                {boilerNames.map((name) => (
                  <li key={name}>
                    <FaSquare className="text-[0.5em]" />
                    {name}
                  </li>
                ))}
              </ul>
